...
CREATE FUNCTION [dbo].[fn_json2xml] (@json VARCHAR(MAX))
RETURNS XML
AS
BEGIN
	RETURN CONVERT(XML, '<root>'+dbo.fn_parse_json2text(REPLACE(REPLACE(@json, '&', ''), '', ''))+'</root>')
END
...
CREATE FUNCTION [dbo].[fn_parse_json2text] (
    @json    varchar(max))
RETURNS varchar(max)
AS
    ...
    WHILE (@json!='') BEGIN;
        ...
        WHILE (@array IS NOT NULL) BEGIN;
            ...
            IF (@value IS NULL AND LEFT(@json, 1)='{') BEGIN;
                ...
                SET @value=CAST(
                    dbo.fn_parse_json2text(LEFT(@json, @offset))
                        AS varchar(max));
